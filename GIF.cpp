...
{
    auto curInputPath = WraapingQuotes(inputFile);
    auto curPalettePath = WraapingQuotes( std::string(outputFile) + ".png");
    auto curOutputPath = WraapingQuotes(std::string(outputFile) + ".gif");
    ...
    std::thread makeThread(&GIF::MakeThread, paletteCommand, gifCommand);

    makeThread.detach();
    return 0;
}

int GIF::MakeThread(const std::string paletteCommand, const std::string gifCommand)
{
    ...
    if (!CreateProcessA(
        ...
        const_cast<LPSTR>(paletteCommand.c_str()), // 명령어 (LPSTR 형식으로 변환)
        NULL,                    // 프로세스 보안 속성
        NULL,                    // 스레드 보안 속성
        FALSE,                   // 표준 핸들 상속 여부
        0,                       // 실행 옵션
    ...
    if (!CreateProcessA(
        NULL,                    // 애플리케이션 경로 (NULL은 명령어로 사용)
        const_cast<LPSTR>(gifCommand.c_str()), // 명령어 (LPSTR 형식으로 변환)
        NULL,                    // 프로세스 보안 속성
        NULL,                    // 스레드 보안 속성
        FALSE,                   // 표준 핸들 상속 여부
        0,                       // 실행 옵션
